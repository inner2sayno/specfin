...
export default function Header() {
  ...
  return (
    <header className="w-full sticky top-0 z-50">
      <div className={`w-full transition-all duration-300 border-b ${isScrolled ? "bg-white/95 backdrop-blur-md shadow-sm border-[#E8EDF4]" : "bg-white border-[#E8EDF4]"}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4">
          <div className="flex items-center justify-between gap-8">
            <Link href="/" className="flex-shrink-0 flex items-center" onClick={closeMobile}>
              <Image src="/logo/logo-dark.png" alt="Specfin Technologies" width={110} height={32} className="h-8 w-auto" priority
              ...
              {NAV_ITEMS.map((item) => {
                ...
                return (
                  <Link key={item.href} href={item.href}
                    className={`px-4 py-2 rounded-md text-[14px] font-medium transition-colors ${isActive ? "text-[#0D3880] bg-[#0D3880]/8 font-semibold" : "text-[#4A5568] hover:text-[#0D3880] hover:bg-[#0D3880]/5"}`}>
                    {item.label}
                  </Link>
                );
              })}
            ...
            <div className="hidden lg:flex items-center gap-2">
              ...
              <div className="flex items-center rounded-md border border-[#E8EDF4] overflow-hidden mr-1">
                <button onClick={() => setLang("en")}
                  className={`px-2.5 py-1.5 text-[12px] font-bold transition-colors ${lang === "en" ? "bg-[#0D3880] text-white" : "text-[#4A5568] hover:bg-[#F7F9FC]"}`}>
                  EN
                </button>
                <button onClick={() => setLang("ko")}
                  className={`px-2.5 py-1.5 text-[12px] font-bold transition-colors ${lang === "ko" ? "bg-[#0D3880] text-white" : "text-[#4A5568] hover:bg-[#F7F9FC]"}`}>
                  한국어
                </button>
              </div>
              {isConnected ? (
                <Link href="/dashboard" className="px-5 py-2.5 rounded-md text-[14px] font-bold bg-[#0D3880] text-white hover:bg-[#1a4fa0] transition">
                  {t("nav_dashboard")}
                </Link>
              ) : (
                <>
                  <Link href="/join?mode=signin" className="px-5 py-2.5 rounded-md text-[14px] font-semibold border border-[#E8EDF4] text-[#0B1628] hover:border-[#0D3880]/40 hover:text-[#0D3880] transition">
                    {t("nav_signin")}
                  </Link>
                  <Link href="/join" className="px-5 py-2.5 rounded-md text-[14px] font-bold bg-[#0D3880] text-white hover:bg-[#1a4fa0] transition shadow-sm">
                    {t("nav_join")}
                  </Link>
                </>
              )}
            ...
            <div className="lg:hidden flex items-center gap-2">
              <div className="flex items-center rounded-md border border-[#E8EDF4] overflow-hidden">
                <button onClick={() => setLang("en")}
                  className={`px-2 py-1 text-[11px] font-bold transition-colors ${lang === "en" ? "bg-[#0D3880] text-white" : "text-[#4A5568]"}`}>EN</button>
                <button onClick={() => setLang("ko")}
                  className={`px-2 py-1 text-[11px] font-bold transition-colors ${lang === "ko" ? "bg-[#0D3880] text-white" : "text-[#4A5568]"}`}>KO</button>
              </div>
              <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className="text-[#0B1628] p-2" aria-label="Toggle menu">
        ...
        {isMobileMenuOpen && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              className="fixed inset-0 bg-black/20 backdrop-blur-sm lg:hidden z-40" onClick={closeMobile} />
            <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -8 }} transition={{ duration: 0.2 }}
              ...
              <nav className="flex flex-col px-5 py-4 gap-0.5">
                ...
                <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.25 }} className="pt-4 flex flex-col gap-2">
                  {isConnected ? (
