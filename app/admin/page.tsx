...
export default function AdminPage() {
  ...
  const inp = (style?: React.CSSProperties): React.CSSProperties => ({
    ...
    padding: '10px 14px', color: '#f1f5f9', fontSize: 14, boxSizing: 'border-box', ...style,
  });
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0f172a', color: '#f1f5f9', fontFamily: 'system-ui, sans-serif' }}>
      <div style={{ background: '#1e293b', borderBottom: '1px solid #334155', padding: '0 32px', display: 'flex', alignItems: 'center', gap: 24, height: 60 }}>
        ...
        {(['overview','products','investments','site-config'] as Tab[]).map(t => (
          <button key={t} onClick={() => setTab(t)} style={{ padding: '0 16px', height: 60, border: 'none', cursor: 'pointer', fontWeight: 600, fontSize: 13, background: 'transparent', color: tab === t ? '#fff' : '#64748b', borderBottom: tab === t ? '2px solid #00A896' : '2px solid transparent', transition: 'all 0.15s' }}>
            {t === 'overview' ? '📊 Overview' : t === 'products' ? '📦 Products' : t === 'investments' ? '💰 Investments' : '⚙️ Site Config'}
          </button>
        ))}
      ...
      <div style={{ maxWidth: 1300, margin: '0 auto', padding: '32px 24px' }}>
        {loading ? (
          <div style={{ textAlign: 'center', padding: 80, color: '#64748b' }}>Loading data...</div>
        ) : (
          <>
            {tab === 'overview' && (
              <div>
                <h1 style={{ fontSize: 26, fontWeight: 700, marginBottom: 4 }}>Dashboard</h1>
                <p style={{ color: '#64748b', marginBottom: 28, fontSize: 14 }}>Platform overview and key metrics</p>
                      ...
                      return (
                        <div key={p.id} style={{ marginBottom: 14 }}>
                          ...
                          <div style={{ background: '#334155', borderRadius: 99, height: 6 }}>
                            <div style={{ background: '#00A896', width: pct + '%', height: 6, borderRadius: 99, transition: 'width 0.5s' }} />
                          </div>
                    ...
                    {investments.length === 0 ? (
                      ...
                      <div key={inv.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '9px 0', borderBottom: '1px solid #334155' }}>
                        <div>
                          ...
                          <div style={{ fontSize: 11, color: '#64748b' }}>{inv.paymentMethod.toUpperCase()} · {new Date(inv.createdAt).toLocaleDateString()}</div>
                        </div>
                  ...
                  <div style={{ background: '#1e293b', borderRadius: 12, padding: 28, marginBottom: 24, border: '1px solid #334155' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16, marginBottom: 16 }}>
                      {[
                        ...
                        { k: 'stage', l: 'Stage (e.g. PHASE 1)' }, { k: 'deadline', l: 'Deadline (YYYY-MM-DD)' },
                        { k: 'expectedReturn', l: 'Expected Return (e.g. 18-28% APY)' }, { k: 'logoUrl', l: 'Logo URL' },
                      ...
                      {[
                        { k: 'targetRaise', l: 'Target Raise (USD)' }, { k: 'minInvestment', l: 'Min Investment (USD)' }, { k: 'investorCount', l: 'Investor Count' },
                      ].map(f => (
                        <div key={f.k}>
                          <label style={{ display: 'block', fontSize: 12, color: '#94a3b8', marginBottom: 5 }}>{f.l}</label>
                    ...
                    <div style={{ marginBottom: 14 }}>
                      ...
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 8 }}>
                        {(editProduct.highlights || []).map((h, i) => (
                          <span key={i} style={{ background: '#334155', borderRadius: 6, padding: '4px 10px', fontSize: 13, display: 'flex', alignItems: 'center', gap: 6 }}>
                            {h}
                            <button onClick={() => setEditProduct(p => ({ ...p, highlights: (p!.highlights||[]).filter((_,j)=>j!==i) }))} style={{ background: 'none', border: 'none', color: '#ef4444', cursor: 'pointer', fontSize: 14, padding: 0, lineHeight: 1 }}>×</button>
                          </span>
                        ))}
                      </div>
                      <input value={hlInput} onChange={e => setHlInput(e.target.value)}
                        ...
                        placeholder="Type highlight and press Enter" style={{ background: '#0f172a', border: '1px solid #334155', borderRadius: 8, padding: '8px 12px', color: '#f1f5f9', fontSize: 13, width: 280 }} />
                    </div>
                    <div style={{ marginBottom: 14 }}>
            ...
            {tab === 'investments' && (
              <div>
                ...
                <p style={{ color: '#64748b', fontSize: 14, marginBottom: 20 }}>All investment records — {investments.length} total, {fmt(totalInvested)} confirmed</p>
                {investments.length === 0 ? (
                  <div style={{ background: '#1e293b', borderRadius: 12, padding: 60, textAlign: 'center', color: '#64748b' }}>
                    <div style={{ fontSize: 40, marginBottom: 12 }}>📭</div>
                    <p>No investments yet. They appear here when users submit investments.</p>
                  </div>
                ) : (
                      ...
                      <thead>
                        <tr style={{ background: '#0f172a' }}>
                          {['Investor','Product','Amount','Method','Status','TX / Wallet','Date'].map(h => (
                            <th key={h} style={{ padding: '12px 14px', textAlign: 'left', fontSize: 11, color: '#64748b', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.05em' }}>{h}</th>
                          ))}
                        </tr>
                      </thead>
                      <tbody>
                        {investments.map((inv, idx) => (
                          <tr key={inv.id} style={{ borderTop: '1px solid #334155', background: idx % 2 === 0 ? '#1e293b' : '#1a2537' }}>
                            ...
                            <td style={{ padding: '12px 14px', fontWeight: 700, color: '#10b981', fontSize: 14 }}>{fmt(inv.amount)}</td>
                            <td style={{ padding: '12px 14px' }}>
            ...
            {tab === 'site-config' && config && (
              <div>
                ...
                <p style={{ color: '#64748b', fontSize: 14, marginBottom: 24 }}>Control public-facing content and site behavior</p>
                <div style={{ display: 'grid', gap: 20, maxWidth: 700 }}>
                  <div style={{ background: '#1e293b', borderRadius: 12, padding: 24 }}>
                    ...
                    <div style={{ display: 'grid', gap: 14 }}>
                      {[{k:'heroTitle',l:'Hero Title'},{k:'heroCta',l:'CTA Button Text'}].map(f => (
                        <div key={f.k}>
                          <label style={{ display: 'block', fontSize: 12, color: '#94a3b8', marginBottom: 5 }}>{f.l}</label>
                          <input value={(config as Record<string,string>)[f.k] || ''} onChange={e => setConfig(c => c ? {...c,[f.k]:e.target.value} : c)} style={inp()} />
                        </div>
                      ))}
                      <div>
                  ...
                  <div style={{ background: '#1e293b', borderRadius: 12, padding: 24 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 12 }}>
                      <input type="checkbox" id="ann" checked={config.announcementEnabled} onChange={e => setConfig(c => c ? {...c,announcementEnabled:e.target.checked} : c)} />
                      <label htmlFor="ann" style={{ fontSize: 14, cursor: 'pointer' }}>Enable announcement banner</label>
                    </div>
                    <input value={config.announcementBanner} onChange={e => setConfig(c => c ? {...c,announcementBanner:e.target.value} : c)} style={inp()} placeholder="Banner text..." />
                  </div>
                  <div style={{ background: '#1e293b', borderRadius: 12, padding: 24 }}>
                    <h2 style={{ fontSize: 15, fontWeight: 700, marginBottom: 16, color: '#f59e0b' }}>🔧 Maintenance Mode</h2>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                      <input type="checkbox" id="maint" checked={config.maintenanceMode} onChange={e => setConfig(c => c ? {...c,maintenanceMode:e.target.checked} : c)} />
                      <label htmlFor="maint" style={{ fontSize: 14, cursor: 'pointer', color: '#ef4444' }}>Enable maintenance mode (blocks public site access)</label>
                    </div>
                  </div>
