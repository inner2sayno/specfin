    ...
    <div className="min-h-screen bg-[#030812] text-white">
      ...
      <div className="relative w-full h-52 overflow-hidden">
        ...
        <div className="absolute bottom-5 left-6 flex items-end gap-4">
          <div className="w-16 h-16 rounded-xl overflow-hidden border-2 border-white/20 bg-[#030812]">
            <Image src={opp.logo} alt="" width={64} height={64} className="object-contain p-2" />
          </div>
          <div>
            ...
            <div className="flex items-center gap-2 mt-1">
              ...
              <span className={`text-[11px] font-semibold px-2 py-0.5 rounded ${statusColors[opp.status]}`}>{opp.statusLabel}</span>
            </div>
          </div>
        </div>
        <Link href="/dashboard/opportunities" className="absolute top-4 left-4 text-[12px] font-semibold text-white/70 hover:text-white flex items-center gap-1">
          ← Back
        </Link>
      </div>
        ...
        <div className="flex flex-col gap-4">
          ...
          {opp.raised > 0 && (
            <div className="rounded-xl border border-white/10 bg-white/[0.04] p-5">
              ...
              <div className="flex justify-between text-[12px]">
                <span className="font-mono font-bold text-white">${"{(opp.raised/1000000).toFixed(2)}"}M raised</span>
                <span className="text-[#9fb6d0]">${"{(opp.target/1000000).toFixed(1)}"}M target</span>
              </div>
            </div>
          )}
          {/* Stats */}
          ...
          {opp.status === "coming-soon" && (
            <div className="rounded-xl border border-[#D97706]/30 bg-[#D97706]/10 p-5 text-center">
              ...
              <p className="text-[12px] text-[#9fb6d0] mb-3">This opportunity launches in Phase 2. Join the waitlist for priority access.</p>
