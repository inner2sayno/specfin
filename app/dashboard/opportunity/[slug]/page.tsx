  ...
  return (
    <div className="min-h-screen bg-[#030812] text-white">
      <div className="relative w-full h-52 overflow-hidden">
        ...
        <Link href="/dashboard/opportunities" className="absolute top-4 left-4 text-[12px] font-semibold text-white/70 hover:text-white flex items-center gap-1 z-10">← Back</Link>
        ...
        <div className="flex flex-col gap-4">
          {opp.raised > 0 && (
            <div className="rounded-xl border border-white/10 bg-white/[0.04] p-5">
              <div className="flex justify-between text-[12px] text-[#9fb6d0] mb-2"><span>Raised</span><span>{pct.toFixed(0)}%</span></div>
          ...
          {opp.status === "open" && !invested && (
            <div className="rounded-xl border border-[#36E8CA]/30 bg-[#081321] p-5">
              ...
              <input type="number" placeholder="Amount (USDT)" value={amount} onChange={(e) => setAmount(e.target.value)} className="w-full bg-white/[0.06] border border-white/15 rounded-lg px-4 py-3 text-[14px] text-white placeholder-white/30 focus:outline-none focus:border-[#36E8CA]/50 mb-3" />
