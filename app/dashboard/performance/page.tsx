...
export default function PerformancePage() {
  const totalInvested = PORTFOLIO_HOLDINGS.reduce((s,item)=>s+item.invested,0);
  const totalCurrent = PORTFOLIO_HOLDINGS.reduce((s,item)=>s+item.currentValue,0);
      ...
      <div className="px-4 lg:px-6 py-8">
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-8">
          {[
            ...
            {label:"Current value",value:"$"+totalCurrent.toLocaleString(),color:"text-white"},
        ...
        <div className="rounded-2xl border border-white/10 bg-white/[0.04] p-6 mb-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-[16px] font-bold text-white">Portfolio value over time (USDT)</h2>
            <span className="text-[11px] text-[#9fb6d0]">Jan – Jun 2026</span>
          </div>
          <div className="flex items-end gap-3 h-32">
            {CHART_DATA.map(d=>(
              <div key={d.month} className="flex-1 flex flex-col items-center gap-1">
                <div className="w-full rounded-t-sm bg-gradient-to-t from-[#00A896] to-[#36E8CA] transition-all"
                  style={{height: ((d.value-min)/(max-min)*100+20)+"%"}} />
        ...
        <div className="rounded-xl border border-white/10 overflow-hidden">
          <div className="hidden md:grid md:grid-cols-5 bg-white/[0.04] px-5 py-3 text-[11px] font-bold text-[#9fb6d0] uppercase tracking-wider border-b border-white/10">
            <span className="col-span-2">Investment</span>
            <span>Invested</span>
            <span>Current</span>
            <span>YTD Return</span>
          </div>
          {PORTFOLIO_HOLDINGS.filter(item=>item.status==="Active").map(item=>(
