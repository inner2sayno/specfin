...
export default function HoldingsPage() {
  const totalInvested = PORTFOLIO_HOLDINGS.reduce((sum,item)=>sum+item.invested,0);
  const totalCurrent = PORTFOLIO_HOLDINGS.reduce((sum,item)=>sum+item.currentValue,0);
  const totalReturn = ((totalCurrent-totalInvested)/totalInvested)*100;
  const active = PORTFOLIO_HOLDINGS.filter(item=>item.status==="Active");
  const totalPnL = totalCurrent - totalInvested;

  const kpis = [
    {label:"Total invested",value:"$"+totalInvested.toLocaleString(),sub:"USDT",color:"text-white"},
    {label:"Current value",value:"$"+totalCurrent.toLocaleString(),sub:"USDT",color:"text-white"},
    {label:"Total P&L",value:(totalPnL>=0?"+":"")+"$"+Math.abs(totalPnL).toLocaleString(),sub:totalReturn.toFixed(1)+"%",color:totalReturn>=0?"text-[#34d399]":"text-[#f87171]"},
    ...
    <div className="min-h-screen bg-[#030812] text-white">
      ...
      <div className="px-4 lg:px-6 py-8">
        ...
        <div className="rounded-xl border border-white/10 overflow-hidden mb-4">
          ...
          {PORTFOLIO_HOLDINGS.map(item=>(
            <div key={item.id} className="flex flex-col md:grid md:grid-cols-6 px-5 py-4 border-b border-white/5 hover:bg-white/[0.03] transition gap-2 md:gap-0 md:items-center">
              ...
              <div className="font-mono text-[13px] text-white">${"{item.invested.toLocaleString()}"}</div>
              <div className="font-mono text-[13px] text-white">${"{item.currentValue.toLocaleString()}"}</div>
              <div className={`font-mono text-[13px] font-bold ${item.returnPct>=0?"text-[#34d399]":"text-[#f87171]"}`}>{item.returnPct>=0?"+":""}{item.returnPct}%</div>
              <div>
                <span className={`text-[11px] font-semibold px-2 py-0.5 rounded ${item.status==="Active"?"bg-[#059669]/15 text-[#34d399] border border-[#059669]/30":"bg-white/5 text-white/40 border border-white/10"}`}>{item.status}</span>
              </div>
            </div>
          ))}
        </div>
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <Link href="/dashboard/opportunities" className="text-[13px] font-semibold text-[#36E8CA] hover:opacity-80">Browse new opportunities →</Link>
        </div>
      </div>
    </div>
  );
